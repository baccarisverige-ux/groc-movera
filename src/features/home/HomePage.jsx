...
function ListingCard({ item, sectionId, favorite, toggleFavorite, index, onNavigate }) {
  ...
  return (
    <MotionListItem as="article" className="b225-offer-card" config={HOME_OFFER_MOTION} index={index} data-testid={`home-card-${sectionId}-${item.id}`} data-origin={item.origin || 'seed'} aria-label={`${item.title}, ${price}, ${rating}`} role="link" tabIndex={0} onClick={openListing} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); openListing() } }}>
      <div className="b225-offer-card__image">
        ...
        <button type="button" className="b225-offer-card__heart" data-active={favorite ? 'true' : 'false'} aria-pressed={favorite} aria-label={`${favorite ? 'Retirer' : 'Ajouter'} ${item.title} ${favorite ? 'des' : 'aux'} favoris`} onClick={(event) => { event.stopPropagation(); toggleFavorite(item.id) }}>{favorite ? '♥' : '♡'}</button>
      </div>
      <div className="b225-offer-card__body"><h3 className="b225-offer-card__title">{item.title}</h3><p className="b225-offer-card__dates">{dates || item.location}</p><p className="b225-offer-card__meta"><strong>{price}</strong><span className="b225-offer-card__dot">·</span><span>{rating}</span></p></div>
    </MotionListItem>
  )
}
...
export function HomePage({ onNavigate }) {
  ...
  useEffect(() => {
    const sync = () => { setHostTick((value) => value + 1); setLastSearch(readLastSearch()) }
    window.addEventListener(HOST_PROFILE_EVENT, sync); window.addEventListener('storage', sync); window.addEventListener('focus', sync)
    return () => { window.removeEventListener(HOST_PROFILE_EVENT, sync); window.removeEventListener('storage', sync); window.removeEventListener('focus', sync) }
  }, [])
  ...
  const categorySelections = useMemo(() => { void hostTick; return homeCategories.map((item) => ({ id: item.id, title: item.label, items: listHomeOffersByCategory(item.id) })) }, [hostTick])
  ...
  const openLastSearch = () => { const path = lastSearchPath(lastSearch); if (path) onNavigate(path) }
  const renderSelection = (selection) => <CategorySelection key={selection.id} id={selection.id} title={selection.title} items={selection.items} favoriteIdSet={favoriteIdSet} toggleFavorite={toggleFavorite} onNavigate={onNavigate}/>
    ...
    <div className="b225-home" data-testid="page-home">
      <header className="b225-home-header">
        ...
        <div className="b225-home-search-shell" data-has-last-search={lastSearch ? 'true' : 'false'}>
          <button type="button" className="b225-search b225-home-map-search" data-testid="home-search" aria-label="Rechercher une destination"><SearchIcon/><span className="b225-home-map-search__copy"><strong>Explorez autrement</strong><span>Destination · Dates · Voyageurs</span></span>{!lastSearch ? <span className="b225-home-map-filter-button" aria-hidden="true">≡</span> : null}</button>
          {lastSearch && lastSearchPreview.length ? (
            <button type="button" className="b225-last-search-stack" data-testid="home-last-search" onClick={openLastSearch} aria-label={`Reprendre la dernière recherche ${lastSearch.label || ''}`}>
              <span className="b225-last-search-stack__label"><i aria-hidden="true">↻</i><span>Dernière<br/>recherche</span></span>
              <span className="b225-last-search-stack__cards" aria-hidden="true">{lastSearchPreview.map((item, index) => <span key={`last-search-${item.id}`} className="b225-last-search-stack__card" style={{ '--last-search-index': index }}><img src={item.image} alt="" decoding="async"/></span>)}</span>
            </button>
          ) : null}
        </div>
      </header>
