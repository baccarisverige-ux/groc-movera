...
const WELCOME_CITIES = [
  { id: 'sidi-bou-said', label: 'Sidi Bou Saïd' }, { id: 'sousse', label: 'Sousse' }, { id: 'hammamet', label: 'Hammamet' }, { id: 'tunis', label: 'Tunis' }, { id: 'djerba', label: 'Djerba' }, { id: 'tozeur', label: 'Tozeur' }, { id: 'tabarka', label: 'Tabarka' },
]
...
function ListingCard({item,sectionId,favorite,toggleFavorite,index,onNavigate}){const dates=item.dates||item.dateLabel||'';const price=item.priceLabel||item.priceTotal||(item.nightlyRate!=null?`${item.nightlyRate} ${item.currency||'TND'} / nuit`:'Tarif à confirmer');const rating=item.rating?`★ ${item.rating}`:'Nouveau';const openListing=()=>onNavigate(`/listing/${item.id}`);return <MotionListItem as="article" className="b225-offer-card" config={HOME_OFFER_MOTION} index={index} data-testid={`home-card-${sectionId}-${item.id}`} data-origin={item.origin||'seed'} aria-label={`${item.title}, ${price}, ${rating}`} role="link" tabIndex={0} onClick={openListing} onKeyDown={(event)=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();openListing()}}}><div className="b225-offer-card__image">{item.image?<img src={item.image} alt="" loading="lazy" decoding="async"/>:<span aria-hidden="true">MH</span>}{item.badge?<span className="b225-offer-card__badge">{item.badge}</span>:null}<button type="button" className="b225-offer-card__heart" data-active={favorite?'true':'false'} aria-pressed={favorite} aria-label={`${favorite?'Retirer':'Ajouter'} ${item.title} ${favorite?'des':'aux'} favoris`} onClick={(event)=>{event.stopPropagation();toggleFavorite(item.id)}}>{favorite?'♥':'♡'}</button></div><div className="b225-offer-card__body"><h3 className="b225-offer-card__title">{item.title}</h3><p className="b225-offer-card__dates">{dates||item.location}</p><p className="b225-offer-card__meta"><strong>{price}</strong><span className="b225-offer-card__dot">·</span><span>{rating}</span></p></div></MotionListItem>}
function SeeAllCard({id,title,items,onNavigate}){const previewItems=items.slice(-3);const openAll=()=>{setSelectedHomeCategory(id);const route=getCollectionRouteForCategory(id);onNavigate(route||`/map?category=${encodeURIComponent(id)}`)};return <button type="button" className="b225-see-all-card" data-category={id} data-testid={`home-see-all-${id}`} onClick={openAll} aria-label={`Tout voir dans ${title}`}><span className="b225-see-all-card__gallery" aria-hidden="true">{previewItems.map((item)=><span key={`${id}-preview-${item.id}`} className="b225-see-all-card__photo">{item.image?<img src={item.image} alt="" loading="lazy" decoding="async"/>:<span>MH</span>}</span>)}<span className="b225-see-all-card__mark">M</span></span><span className="b225-see-all-card__copy"><span className="b225-see-all-card__eyebrow">Movera Select</span><span className="b225-see-all-card__title">Tout voir</span><span className="b225-see-all-card__arrow" aria-hidden="true">→</span></span></button>}
