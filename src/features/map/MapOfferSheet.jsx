...
function lotPrice(lot, listing) {
  return lot?.basePrice ? `${lot.basePrice} ${lot.currency || listing.currency || 'TND'} / nuit` : listingPriceCopy(listing)
}

function lotMeta(lot) {
  if (!lot) return ''
  const parts = []
  if (lot.sizeM2) parts.push(`${lot.sizeM2} m²`)
  parts.push(`${lot.guests} voy.`)
  parts.push(`${lot.beds} lit${lot.beds > 1 ? 's' : ''}`)
  if (lot.view) parts.push(lot.view)
  return parts.join(' · ')
}
...
function MapOfferListCard({ listing, index, total, selected, onOpen }) {
  const lots = listingLots(listing)
  const [selectedLotId, setSelectedLotId] = useState(() => lots[0]?.id || '')
  const selectedLot = lots.find((lot) => lot.id === selectedLotId) || lots[0] || null
  const image = lotImage(selectedLot, listing)

  useEffect(() => { setSelectedLotId(lots[0]?.id || '') }, [listing.id])

  const open = () => onOpen(listing.id, selectedLot?.id || '')
  ...
  return (
    <MotionListItem
      ...
      data-room-lot-id={selectedLot?.id || ''}
      data-active={selected ? 'true' : 'false'}
      onClick={open}
      onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); open() } }}
    >
      <span className="map-offer-sheet__media">
        <img src={image} alt="" loading={index < 2 ? 'eager' : 'lazy'} />
        {lots.length > 1 ? <span className="map-offer-sheet__multi-badge">{lots.length} catégories</span> : listing.badge ? <span className="map-offer-sheet__badge">{listing.badge}</span> : null}
        <span className="map-offer-sheet__position" aria-hidden="true">{index + 1}/{total}</span>
      </span>
      <span className="map-offer-sheet__card-copy">
        <span className="map-offer-sheet__card-head">
          <span><strong>{listing.title}</strong><small>{listing.location}, Tunisie</small></span>
          <span className="map-offer-sheet__rating"><StarIcon />{listing.rating}</span>
        </span>

        {lots.length > 1 ? (
          <span className="map-offer-sheet__lot-switch" role="radiogroup" aria-label={`Catégories de chambres de ${listing.title}`} onClick={(event) => event.stopPropagation()} onKeyDown={(event) => event.stopPropagation()}>
            {lots.map((lot) => <button type="button" role="radio" aria-checked={lot.id === selectedLot?.id} data-active={lot.id === selectedLot?.id ? 'true' : 'false'} key={lot.id} onClick={() => setSelectedLotId(lot.id)}><b>{lot.name}</b><small>{lot.basePrice} {lot.currency || listing.currency || 'TND'}</small></button>)}
          </span>
        ) : null}

        {selectedLot ? <span className="map-offer-sheet__lot-summary"><b>{selectedLot.name}</b><small>{lotMeta(selectedLot)}</small>{selectedLot.description ? <em>{selectedLot.description}</em> : null}</span> : null}
        <span className="map-offer-sheet__price"><b>{lotPrice(selectedLot, listing)}</b></span>
      </span>
    </MotionListItem>
  )
}
      ...
      <div className="map-offer-sheet__panel" data-attachment-state={attached ? 'attached' : 'moving'} data-idle-hint={idleHintActive ? 'true' : 'false'}>
        <div ref={dragZoneRef} className="map-offer-sheet__drag-zone" data-testid="map-offer-sheet-handle" data-attachment-state={attached ? 'attached' : 'moving'} data-header-offset={Math.round(safeHeaderHeight)} onPointerDown={(event) => { if (event.pointerType !== 'touch') startDrag(event) }}>
          <button type="button" className="map-offer-sheet__handle-button" onClick={toggleExpanded} aria-label={progress > 0.72 ? 'Réduire la liste des offres' : 'Afficher la liste des offres'}>
            ...
            <span className="map-offer-sheet__heading"><strong>{displayedListings.length ? `${displayedListings.length} offre${displayedListings.length > 1 ? 's' : ''}` : 'Aucune offre'}</strong><span className="map-offer-sheet__city-label">{cityLabel}</span><span className="map-offer-sheet__brand-badge">Movera Host</span></span>
        ...
        {displayedListings.length ? (
          <MotionList nodeRef={listRef} className="map-offer-sheet__list" data-scroll-enabled={attached ? 'true' : 'false'} data-motion-list="map-offers" data-map-scroll="independent" data-sheet-handoff="drag-from-offer">
            <div className="map-offer-sheet__list-content" data-testid="map-offer-sheet-list-content">
              {displayedListings.map((listing, index) => <MapOfferListCard key={listing.id} listing={listing} index={index} total={displayedListings.length} selected={listing.id === selectedListingId || (!selectedListingId && index === 0 && progress > 0.12)} onOpen={openListing}/>) }
            </div>
          </MotionList>
        ) : <div className="map-offer-sheet__empty"><div className="map-offer-sheet__empty-content"><strong>Aucune offre Movera dans cette ville</strong><span>La carte reste disponible pour explorer la zone.</span></div></div>}
      </div>
    </>
  )
}
