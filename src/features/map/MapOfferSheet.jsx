  ...
  return <>
    ...
    <motion.div className="map-offer-sheet__panel" data-attachment-state={attached ? 'attached' : 'moving'} data-idle-hint={idleHintActive ? 'true' : 'false'} style={{ y: panelOffsetY, marginBottom: -safeHeaderHeight }}>
      ...
      {displayedListings.length ? <MotionList nodeRef={listRef} className="map-offer-sheet__list" data-scroll-enabled={attached ? 'true' : 'false'} data-motion-list="map-offers" data-map-scroll="independent" data-sheet-handoff="drag-from-offer"><div className="map-offer-sheet__list-content" data-testid="map-offer-sheet-list-content">{displayedListings.map((listing, index) => {
        ...
        return <MotionListItem as="article" key={listing.id} index={index} active={selected} config={MAP_OFFER_ITEM_MOTION} className="map-offer-sheet__card" data-listing-id={listing.id} data-active={selected ? 'true' : 'false'}><button type="button" className="map-offer-sheet__card-main" onClick={() => openListing(listing)} aria-label={`Ouvrir ${listing.title}`}><span className="map-offer-sheet__media"><OptimizedListingImage src={image} alt="" loading={index < 2 ? 'eager' : 'lazy'} sizes="(max-width:430px) 144px, 180px" />{listing.badge && !listing.highlightBadges?.length ? <span className="map-offer-sheet__badge">{listing.badge}</span> : null}<span className="map-offer-sheet__position" aria-hidden="true">{index + 1}/{displayedListings.length}</span></span><span className="map-offer-sheet__card-copy"><span className="map-offer-sheet__card-head"><span><strong>{listing.title}</strong><small>{listing.location}, Tunisie</small></span><span className="map-offer-sheet__rating"><StarIcon/>{listing.rating}</span></span><ListingHighlightBadges badges={listing.highlightBadges} variant="map"/><span className="map-offer-sheet__price"><b>{price}</b></span></span></button>{categorized ? <span className="map-offer-sheet__room-categories map-offer-sheet__room-categories--card"><em>{categories.length} catégories</em><span>{categories.map((room) => <span role="button" tabIndex="0" key={room.id} data-active={room.id === activeRoom?.id ? 'true' : 'false'} onClick={(event) => chooseRoom(event, listing.id, room.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') chooseRoom(event, listing.id, room.id) }}>{room.name}<b>{room.basePrice}</b></span>)}</span></span> : null}<button type="button" className="map-offer-sheet__map-button" data-testid={`map-focus-${listing.id}`} onClick={(event) => focusListingOnMap(event, listing.id)} aria-label={`Voir ${listing.title} sur la carte`}><MapPinIcon/><span>Voir sur la carte</span></button></MotionListItem>
