...
export function resolveHostPhotoUrlSync(ref) {
  if (typeof ref !== 'string' || !ref) return ''
  if (ref.startsWith('data:') || ref.startsWith('blob:') || !isPersistentHostPhotoRef(ref)) return ref
...
export async function resolveHostPhotoUrl(ref) {
  const immediate = resolveHostPhotoUrlSync(ref)
  if (immediate) return immediate
  if (!isPersistentHostPhotoRef(ref)) return typeof ref === 'string' ? ref : ''
  const blob = await readBlob(ref)
  if (!blob) return ''
  return cacheObjectUrl(ref, blob)
}

export async function removeHostPhoto(ref) {
  if (typeof ref !== 'string' || !ref) return
  const cached = objectUrlCache.get(ref)
  if (cached && typeof URL !== 'undefined' && typeof URL.revokeObjectURL === 'function') URL.revokeObjectURL(cached)
