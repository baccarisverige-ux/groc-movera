...
const pagesBaseEnabled = process.env.MOVERA_PAGES_BASE_TEST === '1'

test.describe('GitHub Pages production base path', () => {
  test.skip(!pagesBaseEnabled, 'Runs only with playwright.pages.config.mjs')

  test('survives native navigation and direct routes', async ({ page, context }) => {
    await page.goto('./')
