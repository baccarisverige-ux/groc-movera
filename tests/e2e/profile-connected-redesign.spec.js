...
test('connected profile uses the new Movera hierarchy without duplicating the reference layout', async ({ page }) => {
  ...
  await expect(page.locator('.connected-profile__fab-slot, .connected-profile__host-fab')).toHaveCount(0)
  const hostCta = page.locator('.connected-profile__host-card').getByTestId('switch-to-hosting')
  await expect(hostCta).toBeVisible()
  await expect(hostCta).toContainText('Passer en mode Hôte')
  await expect(hostCta).toHaveCSS('border-radius', '16px')
  await expect(hostCta).toHaveCSS('background-color', 'rgb(49, 95, 76)')
