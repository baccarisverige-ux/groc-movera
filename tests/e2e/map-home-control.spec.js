...
test('map keeps amenity filters fully inside the white header', async ({ page }) => {
  ...
  await expect(amenityFilters.locator('[data-filter-id]')).toHaveCount(7)
  ...
  expect(Math.abs(stageBox.y - (headerBox.y + headerBox.height) + 18)).toBeLessThanOrEqual(2)
  expect(Math.abs(surfaceBox.y - stageBox.y)).toBeLessThanOrEqual(2)
  expect(Math.abs((headerBox.height + stageBox.height) - 862)).toBeLessThanOrEqual(3)
